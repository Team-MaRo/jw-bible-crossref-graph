...
`nwtsty_X.db`, `bible_crossrefs.csv`, `bible_circle.svg`, `bible_circle.png`, `bible_circle.html`.
...
This particular publication is the **German** Study Bible (Studienbibel), so all output is German.
...
Labels/render: `LABEL_FONTSIZE`, `TITLE`, `SUBTITLE`, `FIG_INCHES`, `DPI`.
Outputs: `WRITE_DB`/`WRITE_CSV`/`WRITE_SVG`/`WRITE_PNG`/`WRITE_HTML` toggles + `OUT_*` filenames.

Localization: `BOOK_ABBR` (compact abbreviations used in CSV/verse labels) and `BOOK_NAME` (full
names used for the ring labels). `N_OT_BOOKS = 39` defines the OT/NT split.
